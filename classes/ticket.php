...
class ticket {
    ...
    public static function notify(
        ...
        if (!empty($url)) {
            $ticket = self::generate($recipient, $purpose, $url, $term);
        ...
        if ($CFG->debugsmtp) {
            $e = new stdClass();
            $e->recipient = fullname($recipient);
            $e->notification = $notificationhtml;
            $msg = get_string('debugmsg', 'auth_ticket', $e);
            echo $msg;
        }
    ...
    public static function notifyrole(
        ...
        // M4.
        $fields = fields::for_name()->get_required_fields();
        ...
        foreach ($assigns as $assign) {
            ...
            $ticket = self::generate($user, $purpose, $url, $term);
     ...
     */
    public static function generate($user, $reason, $url, $method = null, $term = 'short', $expires = 0) {
        global $CFG, $DB, $SITE;
        ...
        if ($method == 'internal') {
            ...
            while (core_text::strlen($key) < core_text::strlen($keyinfo)) {
            ...
            for ($i = 0; $i < core_text::strlen($keyinfo); $i++) {
                    $encrypted .= core_text::substr($keyinfo, $i, 1) ^ core_text::substr($key, $i, 1);
            }
        } else if ($method == 'rsa') {
            ...
            if (!empty($CFG->passwordsaltmain)) {
                $pkey = core_text::substr(base64_encode($CFG->passwordsaltmain), 0, 16);
            }
            $sql = "
                SELECT
     ...
     * @SuppressWarnings(PHPMD.CyclomaticComplexity)
     */
    public static function decode($encrypted, $method = null) {
        global $CFG, $DB, $SITE;
        ...
        if ($method == 'internal') {
            ...
            while (core_text::strlen($key) < core_text::strlen($encrypted)) {
                // Pad key onto itself to get a key larger than the text.
                $key .= $key;
            }
            ...
            // Iterate through each character.
            for ($i = 0; $i < core_text::strlen($encrypted); $i++) {
                $decrypted .= core_text::substr($encrypted, $i, 1) ^ core_text::substr($key, $i, 1);
            }
        } else if ($method == 'rsa') {
            // Using RSA.
            ...
            if (!openssl_private_decrypt(urldecode($encrypted), $decrypted, $keypair['privatekey'])) {
                throw new moodle_exception('decodeerror', 'auth_ticket', $method);
            }
        } else {
     ...
     * @SuppressWarnings(PHPMD.CyclomaticComplexity)
     */
    public static function accept($ticket) {
        global $DB, $USER;
